...
| `frontend/src/pages/Login.jsx`     | API calls now prefixed with `API_BASE_URL`                                                          |
...
| `frontend/src/data/lessonsApi.js`  | API calls now prefixed with `API_BASE_URL`                                                           |
...
| `server/requirements.txt`          | Removed duplicate/conflicting version pins (see below)                                              |
...
Getting this deployed on Render (or any host where the frontend and backend live on two different URLs) exposed three problems with how the app was wired for local-only development:

1. **Every fetch call used a relative path** (`/api/auth/...`, `/api/lessons`), which only worked locally because `vite.config.js` proxies `/api` to `http://localhost:8000` in dev. That proxy doesn't exist in a production static build, so relative calls would 404 once frontend and backend are on separate domains.
...
- **In production**, `VITE_API_BASE_URL` is set at build time (e.g. `https://quantum-lab-backend.onrender.com`) → Vite bakes it in as a literal at build time → calls become absolute (`https://quantum-lab-backend.onrender.com/api/auth/login`), reaching the backend directly with no reverse-proxy/rewrite-rule needed on the frontend's static site.
...
- **Frontend** (`quantum-lab-frontend`): static site, `rootDir: frontend`, `npm install && npm run build`, publishes `dist`. Includes the SPA fallback rewrite (`/* → /index.html`) required for React Router routes like `/resources/:id` to survive a page refresh.
...
curl -i -X OPTIONS http://127.0.0.1:8000/api/lessons \
...
npm run build
grep -o "/api/lessons" dist/assets/*.js   # should match
...
2. Log in / register and confirm it still works (still goes through the Vite proxy, `API_BASE_URL` is empty locally).
3. Visit `/resources` and confirm lessons still load from the API.
...
- CORS: started the backend and sent a real `OPTIONS` preflight request with `Origin: http://localhost:5173` — confirmed `access-control-allow-origin: http://localhost:5173` in the response, matching the new default. Confirmed comma-separated `CORS_ORIGINS` parses into a clean list with whitespace trimmed.
- `VITE_API_BASE_URL`: built the frontend twice — once with no env var (confirmed `/api/lessons` appears as a bare relative string in the output bundle) and once with `VITE_API_BASE_URL=https://quantum-lab-backend.onrender.com` set (confirmed that exact URL string appears as a literal in the built bundle, to be joined with `/api/lessons` at runtime via the template literal). Rebuilt a final time without the env var to leave the repo in its normal local-dev-friendly state.
- Confirmed no other hardcoded `/api` string literals remained anywhere in `frontend/src` after the fetch-call updates (`grep -rn "'/api" frontend/src` returned nothing).
